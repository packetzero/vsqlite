...
namespace vsqlite {
  ...
  private:
    ...
    //--------------------------------------------------------------------
    DynType toDynType(int sqliteType) {
      switch(sqliteType) {
...
class VSQLiteRegistryImpl : public VSQLiteRegistry {
public:
  virtual ~VSQLiteRegistryImpl() {
    // TODO
  }
  ...
  virtual int remove(SPVirtualTable spVirtualTable) override {
    return 0;
  }

  virtual int remove(std::string tableName) override {
    return 0;
  }
};

static std::shared_ptr<VSQLiteRegistryImpl> gRegistryInstance;

SPVSQLiteRegistry VSQLiteRegistryInstance() {
  if (nullptr == gRegistryInstance) {
    gRegistryInstance = std::make_shared<VSQLiteRegistryImpl>();
  }
